require "spec"
require "../src/option_parser9"

private def expect_capture_option(args, option, value)
  flag = nil
  OptionParser9.parse(args) do |opts|
...
private def expect_missing_option(option)
  expect_raises OptionParser9::MissingOption do
    OptionParser9.parse([] of String) do |opts|
      opts.on(option, "some flag") do |flag_value|
      end
    end
  end
...
private def expect_missing_option(args, option, flag)
  expect_raises OptionParser9::MissingOption, "Missing option: #{flag}" do
    OptionParser9.parse(args) do |opts|
      opts.on(option, "some flag") do |flag_value|
      end
    end
...
describe "OptionParser9" do
  ...
  describe "gnu_optional_args" do
    it "doesn't get optional argument for short flag after space" do
      flag = nil
      args = %w(-f 123)
      OptionParser9.parse(args, gnu_optional_args: true) do |opts|
        opts.on("-f [FLAG]", "some flag") do |flag_value|
          flag = flag_value
        end
      end
  ...
  it "parses argument when only referenced in long flag" do
    captured = ""
    parser = OptionParser9.parse([] of String) do |opts|
  ...
  it "raises on invalid option" do
    expect_raises OptionParser9::InvalidOption, "Invalid option: -j" do
      OptionParser9.parse(["-f", "-j"]) do |opts|
        opts.on("-f", "some flag") { }
      end
    end
  end
  ...
  it "raises on invalid option if value is given to none value handler (short flag, #9553) " do
    expect_raises OptionParser9::InvalidOption, "Invalid option: -foo" do
      OptionParser9.parse(["-foo"]) do |opts|
        opts.on("-f", "some flag") { }
      end
    end
  end
  ...
  it "raises on invalid option if value is given to none value handler (long flag, #9553)" do
    expect_raises OptionParser9::InvalidOption, "Invalid option: --foo=bar" do
      OptionParser9.parse(["--foo=bar"]) do |opts|
  ...
  it "parses with subcommands twice" do
    ...
    parser = OptionParser9.new do |opts|
      opts.on("subcommand", "") do
        subcommand = true
        opts.on("--foo arg", "") { |v| foo = v }
        opts.on("--bar", "") { bar = true }
  ...
  it "unregisters subcommands on call" do
    ...
    bar = false
    baz = false
    OptionParser9.parse(%w(foo baz)) do |opts|
      opts.on("foo", "") do
        foo = true
        opts.on("bar", "") { bar = true }
      end
  ...
  it "handles subcommand --help well (top level)" do
    help = nil
    OptionParser9.parse(%w(--help)) do |opts|
      opts.banner = "Usage: foo"
  ...
  it "handles subcommand --help well (subcommand)" do
    help = nil
    OptionParser9.parse(%w(subcommand --help)) do |opts|
      opts.banner = "Usage: foo"
      opts.on("subcommand", "Subcommand Description") do
  ...
  it "handles subcommands with hyphen" do
    subcommand = false
    OptionParser9.parse(%w(sub-command)) do |opts|
      opts.banner = "Usage: foo"
...
describe "OptionParser9 with summary_width and summary_indent" do
  it "formats flags and descriptions with default summary_width and summary_indent" do
    parser = OptionParser9.new
