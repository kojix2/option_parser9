# `OptionParser9` is a class for command-line options processing. It supports:
#
...
#
# Run `crystal` for an example of a CLI built with `OptionParser9`.
#
# NOTE: To use `OptionParser9`, you must explicitly import it with `require "option_parser"`
#
# Short example:
#
# ```
...
# # Subcommands
#
# `OptionParser9` also supports subcommands.
#
# Short example:
#
# ```
...
# welcome = false
# name = "World"
# parser = OptionParser9.new do |parser|
...
class OptionParser9
  ...
  def self.parse(args = ARGV, *, gnu_optional_args : Bool = false, &) : self
    parser = OptionParser9.new(gnu_optional_args: gnu_optional_args)
    yield parser
    parser.parse(args)
    parser
  end
  ...
  # require "option_parser"
  #
  # OptionParser9.parse(%w(-a1 -a 2 -a --b=3 --b 4), gnu_optional_args: true) do |parser|
  ...
  # removed from the OptionParser9, simulating a "tree" of subcommands. All flags
  ...
  def parse(args = ARGV) : Nil
    with_preserved_state do
      ...
      while arg_index < args.size
        ...
        # Delegate handler processing and arg_index advancement
        arg_index = process_flag(flag, value, arg_index, args, handled_args)
  ...
  private def parse_flag(arg : String) : {String, String?}
    if arg.starts_with?("--")
      if value_index = arg.index('=')
  ...
  private def process_flag(flag : String, value : String?, arg_index : Int32, args : Array(String), handled_args : Array(Int32)) : Int32
